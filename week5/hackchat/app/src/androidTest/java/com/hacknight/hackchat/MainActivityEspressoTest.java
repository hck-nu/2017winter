...
public class MainActivityEspressoTest {
    ...
    public void testSubscribeAndLog() throws InterruptedException {
        onView(withId(com.hacknight.hackchat.R.id.informationTextView)).check(matches(isDisplayed
                ()));

        // Click subscribe button and check toast
        onView(allOf(withId(com.hacknight.hackchat.R.id.subscribeButton), withText(com.hacknight
                .hackchat.R.string.subscribe_to_news)))
                .check(matches(isDisplayed()))
                .perform(click());
        confirmToastStartsWith(mActivityRule.getActivity().getString(com.hacknight.hackchat.R
                .string.msg_subscribed));
